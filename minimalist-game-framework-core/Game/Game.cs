    ...
    public void Update()
    {
        ...
        if (playing)
        {
            ...
            if (countDown <= countDownConst)
            {
                ...
                MenuSystem.DrawHighlightedString(4 - Math.Floor(countDown) + "",new Vector2(160,60) * ResolutionScale, Color.Black, placeFont, align: TextAlignment.Center);
            ...
            else
            {
                ...
                {
                    float dt = Math.Min(Engine.TimeDelta, 1f / 30f);
                    MenuSystem.DrawHighlightedString("GO!", new Vector2(160,60) * ResolutionScale, Color.Black, placeFont, align: TextAlignment.Center);
                    go += dt;
                }
            }
        }
